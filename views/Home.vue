...
try {
  ...
  console.error('加载 JSON 语言包失败:', error)
        ...
        <div class="usage-guide" v-pre>
          <h4>💡 使用与构建说明：</h4>
          <p>
            1. <strong>表格维护</strong>：直接编辑 <code>i18n/langs.csv</code>。表格首行必须为三列结构：第一列为 <code>key</code>（代码中匹配的中文短句），第二列为 <code>zh-CN</code>（对应的中文渲染值，通常与 key 保持一致），第三列为 <code>en-US</code>（对应的英文翻译）。<br>
...
function scrollToSection(id) {
  ...
  if (el) {
    const yOffset = -90 // 留出 90 像素的高差给悬浮导航条
